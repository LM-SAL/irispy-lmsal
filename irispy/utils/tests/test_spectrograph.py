...
def test_calculate_dn_to_radiance_factor(raster_file, idl_input_rad_cal, idl_output_factor_si):
    ...
    # W m^-2 s^-1 sr^-1 nm^-1.
    idl_factor = idl_factor_si * (u.W / (u.m**2 * u.s * u.sr * u.nm))
    ...
    new_factor = factor * 1.E-3#.to(u.W / (u.m**2 * u.s * u.sr * u.nm))
    np.testing.assert_allclose(new_factor.value, idl_factor.value)